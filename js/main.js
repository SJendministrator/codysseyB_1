...
const navLinks = document.querySelectorAll(".nav-menu a, .hero-buttons a, .logo");
...
const setTheme = (theme) => {
    ...
    themeToggle.setAttribute("title", theme === "dark" ? "라이트 모드로 전환" : "다크 모드로 전환");
...
const renderProjects = () => {
    ...
    if (state.projectStatus === "loading") {
        projectStatus.innerHTML = '<span class="spinner" aria-hidden="true"></span> 프로젝트를 불러오는 중...';
    ...
    projectList.innerHTML = state.projects.map(({ name, description, language, html_url, homepage, updated_at }) => {
        const updatedDate = new Intl.DateTimeFormat("ko-KR", { year: "numeric", month: "short", day: "numeric" }).format(new Date(updated_at));
        const projectDescription = description || "프로젝트 설명이 아직 등록되지 않았습니다.";
        const demoLink = homepage ? `<a class="project-link" href="${homepage}" target="_blank" rel="noopener noreferrer">Live demo <span aria-hidden="true">↗</span></a>` : "";
...
const fetchRepositories = async () => {
    ...
    try {
        ...
        state.projects = repositories.filter(({ fork, archived }) => !fork && !archived);
...
const validateField = (field) => {
    ...
    if (!value) message = `${field.previousElementSibling.textContent.trim()}을(를) 입력해 주세요.`;
    else if (field.type === "email" && !emailPattern.test(value)) message = "올바른 이메일 주소를 입력해 주세요.";
