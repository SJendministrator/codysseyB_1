...
배포된 포트폴리오는 아래 URL에서 확인할 수 있습니다.

[https://sjendministrator.github.io/codysseyB_1/](https://sjendministrator.github.io/codysseyB_1/)
...
| GitHub API 로딩·성공·오류·빈 상태 | 구현 완료 | 코드에 모든 상태와 재시도 버튼이 있으며, 아래 콘솔 명령으로 각 상태를 확인할 수 있습니다. |
| 문의 폼 검증 | 구현 완료 | 이름·이메일·메시지의 required, 이메일 정규식, 필드 근처 오류 메시지, 성공 메시지를 확인했습니다. |
| GitHub Pages 실제 URL | 완료 | 배포 URL을 README에 연결했습니다. |
...
## Projects 상태 테스트 (브라우저 콘솔)

GitHub API는 정상 동작할 때 오류 상태를 보기 어렵습니다. 배포 사이트 또는 Live Server 페이지에서 **F12 → Console**을 열고 아래 명령을 실행하면, 실제 네트워크 오류를 만들지 않고 Projects UI 상태를 의도적으로 전환할 수 있습니다.

```js
portfolioDebug.projects.loading() // 로딩 상태
portfolioDebug.projects.error()   // 오류 상태 + 다시 시도 버튼
portfolioDebug.projects.empty()   // 빈 상태
portfolioDebug.projects.success() // 예시 프로젝트 카드로 성공 상태
portfolioDebug.projects.retry()   // 실제 GitHub API를 다시 요청
```

`error()`와 `empty()`는 테스트용 UI 전환 명령입니다. 실제 API 요청을 실패시키거나 GitHub 데이터에 영향을 주지 않습니다. `retry()` 또는 페이지 새로고침을 실행하면 일반 API 요청 흐름으로 돌아갑니다.
